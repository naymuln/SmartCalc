...
fun SmartCalcTheme(
    ...
    if (!view.isInEditMode) {
        SideEffect {
            ...
            if (window != null) {
                // Set status bar & navigation bar background colors
                window.statusBarColor = colorScheme.background.toArgb()
                window.navigationBarColor = colorScheme.background.toArgb()

                // Set status bar icon contrast:
                // When darkTheme is true: icons are LIGHT/WHITE (isAppearanceLightStatusBars = false)
                // When darkTheme is false: icons are DARK/BLACK (isAppearanceLightStatusBars = true)
